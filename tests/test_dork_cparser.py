...
def test_move_north():
    ...
    assert "move_north" in vars(parser), expect
    assert isinstance(parser.move_north, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.move_north()
    mock_print.assert_has_calls([
...
def test_move_south():
    ...
    """
    expect = "Dork.commandsparser should define a _move_north method"
    assert "move_south" in vars(parser), expect
    assert isinstance(parser.move_south, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.move_south()
    mock_print.assert_has_calls([
...
def test_move_east():
    ...
    assert "move_east" in vars(parser), expect
    assert isinstance(parser.move_east, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.move_east()
    mock_print.assert_has_calls([
...
def test_move_west():
    ...
    assert "move_west" in vars(parser), expect
    assert isinstance(parser.move_west, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.move_west()
    mock_print.assert_has_calls([
...
def test_look_north():
    ...
    assert "look_north" in vars(parser), expect
    assert isinstance(parser.look_north, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.look_north()
    mock_print.assert_has_calls([
...
def test_look_south():
    ...
    assert "look_south" in vars(parser), expect
    assert isinstance(parser.look_south, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.look_south()
    mock_print.assert_has_calls([
...
def test_look_east():
    ...
    assert "look_east" in vars(parser), expect
    assert isinstance(parser.look_east, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.look_east()
    mock_print.assert_has_calls([
...
def test_look_west():
    ...
    assert "look_west" in vars(parser), expect
    assert isinstance(parser.look_west, FunctionType)
    with patch('sys.stdout') as mock_print:
        parser.look_west()
    mock_print.assert_has_calls([
...
def test_evaluate_go_commands(command, response):
    ...
    """
    assert parser.evaluate(command) == response
...
def test_init():
    ...
    """
    with patch.object(parser, "repl", return_value=42):
        with patch.object(parser, "__name__", "__main__"):
            with patch.object(parser.sys, 'exit') as mock_exit:
                parser.init()
