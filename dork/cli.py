...
def evaluate(command, state):
    ...
    if state == State.MENU:
        function = __menu_evaluate(word_list)
    if state == State.GAME:
        function = __game_evaluate(word_list)
    if state == State.LOAD:
        function = __load_evaluate(command)
    if state == State.SAVE:
        function = __save_evaluate()
    return function


def __load_evaluate(path):
